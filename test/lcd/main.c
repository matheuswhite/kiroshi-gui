...
{
    krs_draw_h_line(vram, KRS_POINT(pos.x + size.x, pos.y + shadow + 1), shadow + 1,
                    color);
    krs_draw_h_line(vram, KRS_POINT(pos.x + shadow + 1, pos.y + size.y + shadow + 1),
                    size.x, color);
    krs_draw_v_line(vram, KRS_POINT(pos.x + size.x + shadow + 1, pos.y + shadow + 1),
                    size.y + 1, color);
    krs_draw_v_line(vram, KRS_POINT(pos.x + shadow + 1, pos.y + size.y), shadow + 1,
                    color);
}
...
_Noreturn void menu_page()
{
    ...
    while (1) {
        ...
        krs_draw_bitmap(vram, KRS_POINT(4, 4), battery_full, KRS_POINT(17, 10),
                        TEXT_COLOR);
        krs_draw_bitmap(vram, KRS_POINT(34, 4), battery_66, KRS_POINT(17, 10),
                        TEXT_COLOR);
        krs_draw_bitmap(vram, KRS_POINT(64, 4), battery_33, KRS_POINT(17, 10),
                        TEXT_COLOR);
        krs_draw_bitmap(vram, KRS_POINT(94, 4), battery_empty, KRS_POINT(17, 10), RED);
        ...
        krs_draw_rect_empty(
            vram, KRS_POINT(START_X, START_Y),
...
void draw_inner_line_idx(krs_point_t center, uint16_t inner_circle, uint8_t index)
{
    ...
    switch (index) {
    case 0:
        krs_draw_v_line(vram, KRS_POINT(center.x, center.y - inner_circle), length,
                        TEXT_COLOR);  // s1
        break;
    case 1:
        krs_draw_line(
            vram, KRS_POINT(center.x + inner_circle, center.y - inner_circle),
            KRS_POINT(center.x + (inner_circle / 2), center.y - (inner_circle / 2)),
            TEXT_COLOR);  // d2
        break;
    case 2:
        krs_draw_h_line(vram, KRS_POINT(center.x + inner_circle - length + 1, center.y),
    ...
    case 3:
        ...
        break;
    case 4:
        krs_draw_v_line(vram, KRS_POINT(center.x, center.y + inner_circle - length + 1),
    ...
    case 5:
        ...
        break;
    case 6:
        krs_draw_h_line(vram, KRS_POINT(center.x - inner_circle, center.y), length,
                        TEXT_COLOR);  // s4
        break;
    case 7:
        krs_draw_line(
...
_Noreturn void tape_page(void)
{
    ...
    while (1) {
        ...
        krs_draw_circle(vram, KRS_POINT(center - 40, 60), left_radius, TEXT_COLOR);
        krs_draw_circle(vram, KRS_POINT(center - 40, 60), inner_radius, BACKGROUND_COLOR);
        krs_draw_circle(vram, KRS_POINT(center + 40, 60), right_radius, TEXT_COLOR);
        krs_draw_circle(vram, KRS_POINT(center + 40, 60), inner_radius, BACKGROUND_COLOR);
        krs_draw_line(vram, KRS_POINT(center - 40, 60 - left_radius),
...
{
    krs_list_menu_t menu    = krs_new_list_menu(vram);
    ...
    krs_list_menu_add_many_itens(&menu, itens, sizeof(itens) / sizeof(krs_list_item_t));
